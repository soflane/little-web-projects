...
echo "VITE_ZAMMAD_BASE_URL: '$VITE_ZAMMAD_BASE_URL'"

if [ "$VITE_ZAMMAD_BASE_URL" = "https://your-zammad-instance.com" ]; then
  ...
  ZAMMAD_HOST="$VITE_ZAMMAD_BASE_URL"
  ...
  envsubst '${VITE_ZAMMAD_BASE_URL} ${ZAMMAD_HOST}' < /etc/nginx/conf.d/nginx.conf.template > /etc/nginx/conf.d/default.conf
